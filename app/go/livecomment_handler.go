...
func fillLivecommentResponse(ctx context.Context, tx *sqlx.Tx, livecommentModel LivecommentModel) (Livecomment, error) {
	commentOwnerModel, err := getOrInsertMap(&cache.user, livecommentModel.UserID, func() (UserModel, error) {
		m := UserModel{}
		err := tx.GetContext(ctx, &m, "SELECT * FROM users WHERE id = ?", livecommentModel.UserID)
		return m, err
	})
	if err != nil {
