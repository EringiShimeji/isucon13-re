...
func fillLivestreamResponse(ctx context.Context, tx *sqlx.Tx, livestreamModel LivestreamModel) (Livestream, error) {
	ownerModel, err := getOrInsertMap(&cache.user, livestreamModel.UserID, func() (UserModel, error) {
		m := UserModel{}
		err := tx.GetContext(ctx, &m, "SELECT * FROM users WHERE id = ?", livestreamModel.UserID)
		return m, err
	})
	if err != nil {
