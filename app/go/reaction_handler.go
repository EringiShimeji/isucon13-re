...
func fillReactionResponse(ctx context.Context, tx *sqlx.Tx, reactionModel ReactionModel) (Reaction, error) {
	userModel, err := getOrInsertMap(&cache.user, reactionModel.UserID, func() (UserModel, error) {
		m := UserModel{}
		err := tx.GetContext(ctx, &m, "SELECT * FROM users WHERE id = ?", reactionModel.UserID)
		return m, err
	})
	if err != nil {
		return Reaction{}, err
	...
	livestream, err := getOrInsertMap(&cache.livestream, reactionModel.LivestreamID, func() (Livestream, error) {
